...
/*
    ...
    let b=20;
    let b=30;   오류
    var 타입은 변수를 재생성 할 수 있지만 let는 재생성이 안되는 타입이다.
*/
...
function order(){
    ...
    // 음료에 대한 금액 구하기 - drink_menu라는 함수릁 통해 금액을 구한다.
    ...
    // 사이즈가 m이라면 0원, 1 이라면 1000원

    out += drink.value + " / " + tmp.value + " / " + size.value + " : " + total + "원<br>";

    getId("list").innerHTML=out;

    // 함수 안에 생성된 변수는 함수가 종료 되면 제거 된다.
    // 즉, 함수안에서 생성한 모든 변수는 다시 재사용 할 수 없다.
    // 초기화 되서 사용된다.
