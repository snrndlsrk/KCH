...
function bingo_check(){
    ...
    //state_board의 과일에 클린한 과일배경색 바꾸고
    ...
    for( var i=0; i<Brow; i++){ // 줄
        for(var k=0; k<Bcol; k++){ // 칸
            if(bingo[i*5+k] == 0) // 가로 방향으로 한줄씩 0이 몇개인지 확인
                row++; // 한칸에 0이 있을때마다 1씩 증가
            if(bingo[k*5+i] == 0) // 세로 방향으로 한줄씩 0이 몇개인가
                col++; // 세로 방향 한칸에 0이 있을때마다 1씩 증가
        }
        ...
        if(col==5) end++; // 세로 방향 한줄에 0이 5개라면 1줄 빙고
        row=0;
        col=0;
    }
    if(end==5){
